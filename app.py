"""
╔══════════════════════════════════════════════════════════════════╗
║  CornScan AI  ·  app.py  (Enhanced v4.0 — Fixed)                ║
║  Black landing → White main · CNN Corn Disease Detection         ║
║  TensorFlow / Keras · Streamlit                                  ║
╚══════════════════════════════════════════════════════════════════╝

FIXES APPLIED:
  1. st.experimental_rerun() was outside the if-button block (indentation bug) → moved inside
  2. Replaced all st.rerun() / st.experimental_rerun() with a compatibility
     helper _rerun() that works on ANY Streamlit version (≥ 0.x – 1.x+)
  3. Removed stray/unreachable code after the rerun call in landing_page()
"""
...
""", unsafe_allow_html=True)
    ...
    with c2:
        # ✅ FIX: _rerun() is now correctly INSIDE the if-button block
        if st.button("🌿  Let's Go", use_container_width=True):
            st.session_state.page = "main"
            _rerun()  # <-- was outside the if-block before (indentation bug)
...
<div class="app-topbar">
  <div class="app-topbar-brand">
    ...
    with bc:
        # ✅ FIX: _rerun() replaces st.rerun() for version compatibility
        if st.button("← Home"):
